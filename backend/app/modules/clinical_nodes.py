...
async def get_all_admissions(db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(Admission).options(joinedload(Admission.patient), joinedload(Admission.doctor)))
    return result.scalars().all()
